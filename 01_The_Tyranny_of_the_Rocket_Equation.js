...
const input = require('./01_input');

const part1 = input.reduce((total,mass) => total + (Math.floor(mass/3)-2),0);
...
70 more fuel, which requires 21 fuel, which requires 5 fuel, which requires no further fuel. So, the total fuel required
...
const measure = mass => Math.floor(mass/3)-2;

const part2 = input.reduce((total,mass) => {
  let fuel = measure(mass);
  let subtotal = fuel;
  while (fuel > 6) {
    fuel = measure(fuel);
    subtotal += fuel;
