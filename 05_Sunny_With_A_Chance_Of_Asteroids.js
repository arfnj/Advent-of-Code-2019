...
You're starting to sweat as the ship makes its way toward Mercury. The Elves suggest that you get the air conditioner working
...
Programs that use these instructions will come with documentation that explains what should be connected to the input and
output. The program 3,0,4,0,99 outputs whatever it gets as input, then halts.

Second, you'll need to add support for parameter modes:

Each parameter of an instruction is handled based on its parameter mode. Right now, your ship computer already understands parameter
mode 0, position mode, which causes the parameter to be interpreted as a position - if the parameter is 50, its value is the value
stored at address 50 in memory. Until now, all parameters have been in position mode.

Now, your ship computer will also need to handle parameters in mode 1, immediate mode. In immediate mode, a parameter is interpreted
...
tens digit of the value, that is, the opcode is the rightmost two digits of the first value in an instruction. Parameter modes are
single digits, one per parameter, read right-to-left from the opcode: the first parameter's mode is in the hundreds digit, the second
...
33 * 3 = 99, is written according to the third parameter, 4 in position mode, which also works like it did before - 99 is written to
address 4.
...
function correctly. For each test, it will run an output instruction indicating how far the result of the test was from the expected
value, where 0 means the test was successful. Non-zero outputs mean that a function is not working correctly; check the instructions
...
const raw = require('./05_input');

let i = 0;

const processor = (opcode,param1,param1mode,param2,param2mode,param3) => {
  let value1 = param1mode ? param1 : raw[param1];
  let value2 = param2mode ? param2 : raw[param2];
  if (opcode === 1) {
    raw[param3] = value1 + value2;
  } else {
    raw[param3] = value1 * value2;
  }
}
...
while (i<raw.length && raw[i] !== 99) {
  console.log('i',i);
  if (raw[i] === 3) {
    raw[[raw[i+1]]] = 1;
    i += 2;
  } else if (raw[i] === 4) {
    console.log(raw[[raw[i+1]]]);
    i += 2;
  } else if (raw[i] < 3) {
    processor(raw[i],raw[i+1],0,raw[i+2],0,raw[i+3],0);
    i+=4;
  } else {
    let stringy = raw[i].toString();
    let opcode = Number(stringy.slice(-2));
    if (opcode === 4) {
      console.log(raw[[raw[i+1]]]);
      i+=2;
    } else {
      let params = '000' + stringy.slice(0,-2);
      params = params.slice(-2).split('').reverse();
      processor(opcode,raw[i+1],Number(params[0]),raw[i+2],Number(params[1]),raw[i+3]);
      i += 4
    }
  }
}
...
/*
--- Part Two ---
The air conditioner comes online! Its cold air feels good for a while, but then the TEST alarms start to go off. Since the
air conditioner can't vent its heat anywhere but back into the spacecraft, it's actually making the air inside the ship
warmer.
